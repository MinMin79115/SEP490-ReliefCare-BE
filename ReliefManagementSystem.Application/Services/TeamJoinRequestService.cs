...
namespace ReliefManagementSystem.Application.Services
{
    public class TeamJoinRequestService : ITeamJoinRequestService
    {
        ...
        public async Task<Pagination<TeamJoinRequestResponse>> GetMyRequestsAsync(
            Guid volunteerId,
            int pageIndex,
            int pageSize,
            CancellationToken cancellationToken)
        {
            var query = _unitOfWork.TeamJoinRequests.GetQueryableWithDetails()
                .Where(tjr => tjr.VolunteerId == volunteerId)
                .OrderByDescending(tjr => tjr.CreatedAt);

            var paged = await Pagination<TeamJoinRequest>.ToPagedList(query, pageIndex, pageSize);
            var items = paged.Items!.Select(MapToResponse).ToList();
            return new Pagination<TeamJoinRequestResponse>(items, paged.TotalCount, paged.CurrentPage, paged.PageSize);
        }
        ...
        public async Task<Pagination<TeamJoinRequestResponse>> GetRequestsByTeamAsync(
            Guid teamId,
            Guid moderatorId,
            int pageIndex,
            int pageSize,
